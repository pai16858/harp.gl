...
export class MapView extends EventDispatcher {
    ...
    private setupRenderer() {
        ...
        /**
         * Custom sorting function to avoid non-deterministic IBCT testcases. It's basically a copy of the
         * [three.js internal sorting](https://github.com/mrdoob/three.js/blob/r118/src/renderers/webgl/WebGLRenderLists.js#L5)
         * but additionally checking tile-id before checking object.id, material.id and program.id b/c
         * these ids are generated by incrementing a counter. This means if for two test executions the
         * tiles are processed in a different order the ids hence draw call order will also be different.
         */
        const painterSortStable = (a: RenderItem, b: RenderItem): number => {
            ...
            const isExtrudedPolygonA = techniqueA && isExtrudedPolygonTechnique(techniqueA);
            const isExtrudedPolygonB = techniqueB && isExtrudedPolygonTechnique(techniqueB);
            ...
            //             a.renderOrder +
            //             " at level: " +
            //             (mapObjectAdapterA?.level ?? 0) +
            //             " with road B: " +
            //             b.renderOrder +
            //             " at level: " +
            //             (mapObjectAdapterB?.level ?? 0)
            //     );
            // }
            if (
                ...
                // The following conditions make sure that roads are handled correctly
            } else if (!isRoadA && !isExtrudedPolygonA && isRoadB) {
                return -1;
            } else if (isRoadA && !isRoadB && !isExtrudedPolygonB) {
                return 1;
            } else if (
                !(isRoadA && isRoadB) &&
                ...
                console.log("determined by mortoncode");
                return (
